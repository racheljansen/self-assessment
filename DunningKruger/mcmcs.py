...
def post_large_choice(theta, betas, mut, sigt, mub, sigb, eps, num_correct,len_beta):
    ...
    for beta in betas[:num_correct]:
        post += loglik(theta,beta,eps) + logprior(mub,sigb,beta)
   
    for beta in betas[num_correct:]:
        post += loglikfail(theta,beta,eps)+ logprior(mub,sigb,beta)
            
    return post

# acceptance probability
def acc_prob_choice(theta_old, betas_old, theta_new, betas_new, mut, sigt, mub, sigb, eps, num_correct,len_beta):
    alpha = min(1, np.exp(post_large_choice(theta_new, betas_new, mut, sigt, mub, sigb, eps, num_correct,len_beta)
                          - post_large_choice(theta_old, betas_old, mut, sigt, mub, sigb, eps, num_correct,len_beta)))
    
    return alpha

# MCMC chain function
def MCMC(theta, betas, mut, sigt, mub, sigb, nsteps, len_beta, eps, num_correct):        
    ...
    for i in range(nsteps):
        ...
        alpha1 = acc_prob_choice(theta, betas, theta_new, betas, mut, sigt, mub, sigb, eps, num_correct,len_beta)
        ...
        alpha2 = acc_prob_choice(theta, betas, theta, betas_new, mut, sigt, mub, sigb, eps, num_correct,len_beta)
